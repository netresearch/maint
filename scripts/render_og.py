...
SUBTITLE = {
    "en": "netresearch.github.io/maint — measured, reproducible, updated daily",
    "de": "netresearch.github.io/maint — gemessen, reproduzierbar, täglich aktualisiert",
}
...
def main() -> None:
    for lang in ("en", "de"):
        translations = yaml.safe_load((DASHBOARD / "i18n" / f"{lang}.yaml").read_text(encoding="utf-8"))
        target = DASHBOARD / "assets" / f"og-impact-{lang}.png"
        cairosvg.svg2png(
            bytestring=card(translations["hero"]["title"], SUBTITLE[lang]).encode("utf-8"),
            write_to=str(target),
            output_width=1200,
            output_height=630,
        )
